...
- [x] Real end-to-end proof the mechanism reaches the shared control layer: clicking "Discover devices" in the running application performed a real LAN discovery through `ControlService.discover_devices` and returned real Chromecast devices present on the operator's network (no device names/addresses are recorded in this public file; see the local handoff). No control command (play/pause/stop/seek/volume) has been sent to any real device; the UI's playback controls have only been exercised against a fake TV (see the playback-controls entries below).
...
- [ ] **Volume and mute are not validated on a real Chromecast:** no volume or mute command has been sent to real hardware. This waits for Codex's physical validation of the playback commands, its review and an explicit go-ahead. Two things to watch there, neither changed here: `ControlService` confirms a volume only within `volume_tolerance` (0.01), so a receiver that settles on steps of its own may answer unconfirmed although it acted (the UI then shows the level the TV reports), and `ControlService.set_muted` does not itself check that `muted` is a boolean (the bridge and the PyChromecast adapter do).
- [ ] **Not validated on a real Chromecast:** no play, pause, stop or seek command has been sent to real hardware. This waits for the review of the backend audit (PR #12) and an explicit go-ahead; read-only discovery and status remain the only hardware validation.
...
- [ ] playback controls validated on a real Chromecast (not started: waits for the review of the backend audit (PR #12) and an explicit go-ahead; no command has been sent to a real device);
- [x] volume/mute controls implemented, automation-validated and exercised in the real application against a fake TV;
- [ ] volume/mute validated on a real Chromecast (not started: waits for Codex's physical validation of the playback commands and an explicit go-ahead; no volume or mute command has been sent to a real device);
