...
- [x] Seven bridge methods are exposed so far: three read-only (`ping`, `discover_devices`, `get_status`) and four playback commands (`play`, `pause`, `stop`, `seek`), each a plain forward to the same-named `ControlService` method; every other `TvControl` capability is added to the bridge and to a Tauri command as its own view needs it, not in advance.
...
- [ ] playback controls validated on a real Chromecast (not started: waits for the backend audit, its review and an explicit go-ahead; no command has been sent to a real device);
