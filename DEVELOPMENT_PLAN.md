...
- [ ] **Blocking defect found on real hardware, not fixed in this slice (adapter, owned by the Cast track):** `PyChromecastTransport.discover()` stops PyChromecast's discovery browser (closing its zeroconf instance) in `finally`, so the cached `Chromecast` objects can never connect. With the unmodified adapter every real status read times out ("status read budget exhausted") and the device is then evicted from the cache; a second discovery can also raise `RuntimeError: cannot join thread before it is started` when it disconnects a never-started device. Diagnosed with a read-only script: keeping the browser alive made the same status read succeed in 0.32s, and the UI then displayed the real status - but only with a temporary, uncommitted adapter patch that was reverted. Commands (play, pause, volume) would hit the same connection defect.
...
- [ ] receiver/media state validated on a real device with the unmodified adapter (blocked by the discovery/connection defect above; observed working only with a temporary, uncommitted adapter workaround);
- [ ] playback controls;
...
- [ ] responsive desktop/Android layout (a single-column, touch-sized layout was checked at 390px in a browser harness with a fake backend and in the 480px minimum desktop window; nothing was run on Android);
